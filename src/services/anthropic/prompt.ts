 ...
 *
 * classify.ts와 chat.ts가 이 함수 하나로 system·ledger 블록을 만든다. 분류와
 * Q&A가 같은 프리픽스를 공유해야 하므로, 두 호출부는 이 결과를 그대로 쓰고
 * 자기 목적에 맞는 지시문을 system에 덧붙이지 않는다 — 한 글자라도 달라지면
 * 캐시가 미스되어 비용이 10배가 된다.
 *
